...
#
# 최고1코어 임계 85 의 근거 (2026-08-06 계단 실측)
# ---------------------------------
#   앱 각 방향   최고1코어   앱 손실     80    85    90
#   22 (한계)   66.8~74.7%  ≤0.034%    통과  통과  통과
#   26          79.2%       0.13%      경계  통과  통과
#   30          79.1%       0.79%      경계  통과  통과
#   34 (붕괴)   85.7%       14%        트립  트립  ✗ 미검출
#
# **기본값 90 은 붕괴를 놓쳤다** — 손실 14% 에 실효 미달인데 트립하지 않았다.
# 80 은 79.1~79.2 에 붙어 있어 1초 창 편차(±5~9 포인트)로 정상 구간에서 오탐한다.
# 85 가 깔끔히 갈리고, TCP·RPS 시험에서 붕괴급(85.7%)과 유휴 구간의 s 1코어 100% 를
# 실제로 잡으면서 건강 구간 오탐은 0 이었다.
#
# 이 임계는 "붕괴를 멈추는" 선이지 "손실 0 을 지키는" 선이 아니다. 26~30 구간은
# 동작하지만 손실이 0.1~0.8% 다 — 손실 SLO 를 지키려면 부하 자체를 낮춰야 하고
# 가드로 대신할 수 없다.
set -u
...
CORE=${GW_CORE:-85}             # 어느 호스트든 가장 바쁜 1코어 busy % — 근거는 아래 표
