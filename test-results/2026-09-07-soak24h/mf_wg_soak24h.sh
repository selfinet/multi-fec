...
# 24시간 확장 (2026-09-07)
# ------------------------
# `mf_blast` 는 **종료 시에만** 집계를 출력한다. 24시간을 한 번에 돌리면 중간에
# 죽을 때 전달 집계를 통째로 잃는다(2026-09-05 24 Mbps 런에서 실제로 겪었다).
# 그래서 부하를 CHUNK 단위로 쪼갠다 — 5시간 소크(2026-08-02)와 같은 방식이다.
# 청크별 CSV 가 남아 **"뒤로 갈수록 나빠지는지"** 를 시계열로 볼 수 있다.
# 세션·WG·프로세스는 청크 사이에 그대로 유지된다(재기동하지 않는다).
#
# 가드와 감시자는 **setsid** 로 띄워 이 셸이 끝나도 살아남는다. 가드가 죽으면
# 감시자가 부하를 멈춘다(규칙 0 — 무감시 부하 금지). PID 는 파일로 남겨 정리는
# 문자열 매치가 아니라 PID 로 한다.
#
...
CHUNK=${CHUNK:-$SECS}                                 # 청크 길이(초). 기본은 단일 청크 = 기존 동작
...
echo; echo "=== 9. 샘플러 + 가드 ==="
ssh $C "nohup /tmp/rt_sample.sh 'multi-fec-dist -c -l 127.0.0.1:519' /tmp/wg_c_$RUNID.csv $((SECS+600)) >/dev/null 2>&1 &" 2>/dev/null
ssh $R "nohup /tmp/rt_sample.sh 'multi-fec-dist -r -l 192.168.100.8[56]:$RPORT' /tmp/wg_r_$RUNID.csv $((SECS+600)) >/dev/null 2>&1 &" 2>/dev/null
ssh $S "nohup /tmp/rt_sample.sh 'multi-fec-dist -s -l $SRV:45' /tmp/wg_s_$RUNID.csv $((SECS+600)) >/dev/null 2>&1 &" 2>/dev/null
...
ssh $C "nohup /tmp/rt_sample.sh 'mf_blast' /tmp/wg_blast_$RUNID.csv $((SECS+600)) mf_blast >/dev/null 2>&1 &" 2>/dev/null
setsid nohup $GUARD watch "mf_blast" > $OUT/watchdog.log 2>&1 < /dev/null &
sleep 12
WD=$(ps -eo pid,args | awk '/mf_gwguard\.sh watch/ && !/awk/ {print $1; exit}')
[ -n "${WD:-}" ] && kill -0 $WD 2>/dev/null && { echo "  가드 살아있음 pid=$WD"; echo $WD > $OUT/guard.pid; } \
  || { echo "  ✗ 가드 사망 — 중단"; exit 1; }

echo; echo "=== 10. 부하 ${SECS}s = ${CHUNK}s 청크 × $(( (SECS+CHUNK-1)/CHUNK )) (세션 $N × 각 ${MBPS} Mbps, WG 안) — $(date +%H:%M:%S) ==="
ifsnap load0 > $OUT/ifsnap.txt
( while kill -0 $WD 2>/dev/null; do sleep 10; done
  echo "[$(date +%H:%M:%S)] ✗ 가드 소멸 — 부하 중단" >> $OUT/guard_died.log
  ssh $C "pkill -f '[m]f_blast'" 2>/dev/null ) < /dev/null > /dev/null 2>&1 &
GW=$!; echo $GW > $OUT/supervisor.pid
...
while [ "$(date +%s)" -lt "$T_END" ]; do
  ...
  ssh -o ServerAliveInterval=30 -o ServerAliveCountMax=6 $C \
    ...
    2>>$OUT/blast.err > $OUT/chunk_$(printf %03d $k).csv
  tail -1 $OUT/chunk_$(printf %03d $k).csv | sed 's/^/     /'
  ifsnap "chunk$k" >> $OUT/ifsnap.txt
done
