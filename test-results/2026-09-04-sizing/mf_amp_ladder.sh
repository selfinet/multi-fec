#!/bin/bash
# mf_amp_ladder.sh — **회선 증폭 곡선** 측정 (테스트망 전용)
#
# mf_wg_multi.sh 파생(2026-09-04). 체인을 한 번만 세우고 **레이트만 계단으로 바꿔가며**
# 각 구간의 인터페이스 델타를 잰다. 증폭은 안정성 시험이 아니라 **비율**이므로 구간당
# 5분이면 충분하고, 매 점마다 WG 20개를 세웠다 허무는 비용을 없앤다.
#
# 왜 필요한가: 증폭이 단일 계수가 아니라 **톱니**다.
#   증폭 = FEC(g,y) × 2(duplicate) × 헤더계수     g = 1 + ⌊pps × fec-timeout⌋
#   -f 5:1,20:4 의 패리티 테이블이 계단이라 g 가 경계를 넘을 때 증폭이 되튄다.
#   이론상 R=4 에서 2.40 으로 최소, R=6 에서 2.57 로 되튄다.
# 기존 사이징 식의 단일 계수 2.67 은 이 구조를 무시해 저레이트에서 36% 과소평가했다.
#
# 왜 새로 짰나 (2026-09-02)
# ------------------------
...
N=${N:-1}; RATES=${RATES:-"0.5 1 2 4 6 8"}; STEP=${STEP:-300}   # 세션당 각 방향 Mbps
...
echo "=== 0. 증폭 계단 (세션 $N · 레이트 [$RATES] · 구간 ${STEP}s · 세션별 WG) ==="
echo "  (증폭 계단이라 budget 판정은 생략 — 게이트는 precheck 와 가드)"
...
echo; echo "=== 10. 증폭 계단 — 레이트 $RATES, 구간당 ${STEP}s ==="
...
: > $OUT/steps.txt
for RT in $RATES; do
  echo "  [$(date +%H:%M:%S)] R=$RT 시작"
  sleep 8                                    # 직전 구간 잔류 정리
  ifsnap "s${RT}_a" >> $OUT/ifsnap.txt
  ssh -o ServerAliveInterval=30 -o ServerAliveCountMax=6 $C \
    "/tmp/mf_blast --no-clients --sessions $N --mbps $RT --secs $STEP \
     --src-fmt '10.9.%d.2' --dst-fmt '10.9.%d.1' --net-base $NB --dst-port $ECHOP" \
    2>>$OUT/blast.err | tail -1 | sed "s/^/$RT,/" >> $OUT/steps.txt
  ifsnap "s${RT}_b" >> $OUT/ifsnap.txt
  kill -0 $WD 2>/dev/null || { echo "  ✗ 가드 소멸 — 계단 중단"; break; }
done
kill $GW 2>/dev/null; kill $WD 2>/dev/null
echo "  구간별 전달:"; cat $OUT/steps.txt | sed 's/^/    R=/'
echo "  하네스 건전성(마지막):"; tail -1 $OUT/blast.err
