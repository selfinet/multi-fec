#!/bin/bash
# mf_xver.sh — **구·신 버전 교차 실측** (테스트망 전용)
#
# 왜 (2026-09-11): 운영 바이너리는 6/19 `local-build`(v1.0.1 이전)이고 신규는 v1.3.1 이다.
# 모든 변경이 "와이어 무변경" 으로 기록돼 있고 v1.0.1 때 16종 교차 실측이 있었지만,
# **지금 이 두 바이너리 조합은 확인된 적이 없다.**
#
# mf_wg_multi.sh 를 복사해 역할별 바이너리를 환경변수로 뺐다:
#   BIN_C · BIN_R · BIN_S  (기본 /usr/sbin/multi-fec-dist)
# ⚠️ 호스트에 올리는 구버전 파일 이름을 **`multi-fec-dist` 로 유지**해야 한다 —
#    하네스의 pgrep/pkill 패턴과 샘플러 comm 필터가 그 이름을 본다.
#    그래서 `/tmp/oldver/multi-fec-dist` 처럼 **디렉터리로 구분**한다.
#
# (원본 헤더)
...
BIN_C=${BIN_C:-$BIN}; BIN_R=${BIN_R:-$BIN}; BIN_S=${BIN_S:-$BIN}
...
OUT=${OUT:-/home/stevekim/multi-fec/test-results/2026-09-10-v131/xver_raw}
...
echo "=== 버전 조합 ==="
for R_ in "c:$BIN_C:$C" "r:$BIN_R:$R" "s:$BIN_S:$S"; do
  role=${R_%%:*}; rest=${R_#*:}; bin=${rest%%:*}; host=${rest#*:}
  printf "  %-2s %-34s %s\n" "$role" "$bin" "$(ssh $host "$bin --version 2>&1 | head -1" 2>/dev/null)"
done
...
ssh $S "for i in \$(seq 0 $((N-1))); do
  setsid nohup $BIN_S -s -l $SRV:\$(($SRVP+i)) --wg 127.0.0.1:\$(($WGSP+i)) \
...
for A in $RA $RB; do
  ssh $R "setsid nohup $BIN_R -r -l $A:$RPORT $ROUTES \
...
ssh $C "for i in \$(seq 0 $((N-1))); do
  setsid nohup $BIN_C -c -l 127.0.0.1:\$(($CLIP+i)) \
