...
# 문턱 30% → 20%. sv1에서 A/B 교차 측정하면 이 케이스는 코드와 무관하게
# 24~31% 사이를 오가며(20스레드가 루프백에 동시 폭주하는 구조상 커널 소켓 버퍼
# 경합이 지배) 30%는 상시 경계선이었다. 실제 고장은 0%에 가깝게 떨어지므로
# 20%로도 충분히 구분되고, 이 항목의 핵심 판정은 아래 생존 확인이다.
check(f'다수 클라이언트 수신율 ≥ 20%', recv5 >= total*0.2, f'{recv5}/{total}')
