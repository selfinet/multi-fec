...
```
  ...
  └────────┘      └──────────┘                   └────────────┘
   ...
   +enp3s0 192.168.3.101 ← LTE 라우터              +ens20 1.220.235.146/29
     (2026-08-07 고정IP+정책라우팅. 테스트망 무관)
...
192.168.3.0/24                        dev enp3s0        온링크        (LTE, 테스트망 무관)
# ── table 103 (LTE 전용, 2026-08-07) — main 을 건드리지 않는다
#    netplan routing-policy: from 192.168.3.101/32 → 103
#    systemd `enp3s0-oif-rule.service`: oif enp3s0 → 103 (netplan 은 oif 미지원)
default            via 192.168.3.1    dev enp3s0        table 103    ★ main 아님
...
#### 처리량 — 버스트 한계와 지속 한계는 다르다 (2026-08-07)

| | 앱 각 방향 | 근거 |
|---|---|---|
| **지속 운용 (검증)** | **12 Mbps** | 1시간 소크. c→s 순손실 **0**/4,326,923, s→c 0.0017%, CPU 42.7→43.0% 평탄, 누수 0 |
| 단시간 버스트 | ~22 Mbps | 45초 계단 (2026-08-06). **지속 불가 확인됨** |
| 지속 한계 | 12 < X < 18 | **미측정** |

**각 방향 18 Mbps 는 21분 12초에 무너진다** — 실효는 17.98 Mbps 를 끝까지 유지하는데
**손실이 전반 0.051% → 후반 0.218%(4.3배)로 커지고** c 전체 CPU 가 56%→65% 로 드리프트해
가드가 트립한다(초과 37건 중 30건이 `c:전체>62%`). 열은 아니다(26°C, thermal event 0건).
12 Mbps 에서는 드리프트가 +0.3포인트로 사라지므로 **시간 경과가 아니라 부하 수준에 딸린
현상**이다. 원인은 미규명.

> **45초 창은 이 열화가 나타나기 전에 끝난다.** 계단 시험 수치를 SLA 로 쓰지 말 것 —
> 상한을 "얼마나 빨리 갈 수 있나" 로 재면 처리량은 유지되는데 손실만 무너지는 구간을
> 통째로 놓친다. → `test-results/2026-08-07-aging-1h/REPORT.md`
...
### 29. 클라이언트 pending 큐에 나이 제한이 없다 — 단절 복구 시 27초 묵은 패킷 배달 (2026-09-10, v1.3.1)

**파일**: `mf_client.cpp` — `pending_pkt_t`, `enqueue_pending()`, `flush_pending_packets()`

**증상**: 두 경로가 함께 사라진 뒤(릴레이 호스트 장애 대리) 복구되면 **복구 직후 첫 응답
RTT 가 단절 길이만큼** 나오고, 이후 정확히 프로브 간격씩 감소한다. 2026-09-09 SPOF 시험에서
26,574 ms → 204 ms 씩 감소로 처음 관측했고, 동일 조건 A/B 에서 **27,007 ms** 로 재현했다.

```
복구 후 응답 RTT  27007 → 26803 → 26599 → 26395 …  (204 ms = 프로브 간격)
```

**원인**: `mud_send` 가 EAGAIN 이면 클라이언트는 패킷을 `s_pending_q`(512 엔트리)에 넣는다.
그런데 **엔트리에 시각 필드가 없어** 나이를 알 수 없고, 경로가 살아나는 순간 큐 전체가
나이와 무관하게 배출된다. v1.0.2(§19-라)에서 같은 문제를 서버에는 고쳤는데
(`SERVER_PENDING_TTL_MS`) **클라이언트에는 들어가지 않았다.**

배달이 폐기보다 나쁘다 — 터널 안 TCP 는 그 사이 재전송을 이미 마쳤으므로 묵은 패킷은
**중복**으로 도착해 dup-ACK 와 재정렬을 유발하고, 새 패킷은 그 뒤에 줄을 선다.

**수정**: `pending_pkt_t` 에 `ts` 를 추가하고 `CLIENT_PENDING_TTL_MS 1000` 초과분을
**전송 전에** 버린다.

```c
if (now - pkt.ts > (my_time_t)CLIENT_PENDING_TTL_MS) { /* 폐기 후 다음 엔트리 */ }
int ret = mud_send_mp(g_mud, pkt.data, pkt.len);
```

> **왜 전송 전인가**: 나이 검사를 "전송 실패했을 때" 로 두면 무의미하다. 단절이 끝나면 창이
> 정상으로 돌아오므로 **성공 경로로 백로그가 통째로 통과**한다. 서버(`flush_server_pending()`)
> 가 정확히 그 형태이고, 그래서 아래 잔존 결함이 남는다.

**와이어 무변경 · 클라이언트 단독** — 서버·릴레이를 구버전으로 두고 한쪽만 교체할 수 있다.

**A/B 실측** (같은 스크립트 `pendttl_test.sh`, 클라이언트 바이너리만 교체, 단절 26초):

| | v1.3.0 | v1.3.1 |
|---|---|---|
| 복구 후 첫 응답 RTT | **27,007 / 27,014 ms** | **1,978 / 1,937 / 2,026 ms** |
| 1초 초과 응답 | 60개 이상 | **5~6개** |
| stale 폐기 | 0 (기능 없음) | **745개** |
| 클라 RSS / FD | 11.19 → 13.73 MB / 8 | 11.46 → 13.40 MB / 8 (2사이클에 고원) |

**⚠️ 남은 2초는 multi-fec 이 아니었다 — WireGuard 재핸드셰이크다 (v1.3.2 에서 정정).**
서버 큐를 고쳐도(v1.3.2) 꼬리가 그대로였고, `wg show latest-handshakes` 를 200ms 간격으로
함께 재니 **핸드셰이크 갱신 +2.03s · 응답 일괄 도착 +1.96s · 그 요청들의 송신은 +0.01~1.03s**
였다. 경로 복귀 동안 WG 가 staged 큐(피어당 128패킷)에 담아뒀다가 완료 시 방출한 것이다.
아래 문단은 **반증된 당시 기록**이다.

~~남은 2초는 서버 큐다 (미수정).~~ v1.3.1 의 잔존 꼬리를 송신 시각으로 귀속시키면
(`송신 = 수신 − RTT`) **응답이 한 시점에 몰려 도착하고 송신은 거의 전부 복구 이후**다.
즉 클라이언트가 묵은 패킷을 낸 것이 아니라 **복구 직후 보낸 새 패킷이 귀환 경로에서 한꺼번에
풀린 것**이다 — 단일 flush 의 서명. `flush_server_pending()` 이 TTL 을 `mud_send` 실패
시에만 보기 때문이다. 서버측 수정은 별개 변경이라 이번 범위에서 제외했다(제안: 클라이언트와
같은 전송 전 검사).

**측정 근거**: `test-results/2026-09-10-v131/REPORT.md` §3. 같은 배포본으로 목표 구조
30분 소크도 함께 돌려 누수 없음을 확인했다(왕복 99.9996%, 오배송 0, FD 고정, v1.3.0 궤적과
동일 고원) — 다만 **30분은 검출 창으로 짧다**(v1.3.0 은 24시간).

---

**테스트 스크립트:**
- `pendttl_test.sh` (`test-results/2026-09-10-v131/`) — 클라이언트 pending 큐 TTL 검증. 릴레이 2개 동시 정지→복구를 N회 반복해 **일반 소크가 타지 않는 코드 경로**를 실행시킨다. 분석 `analyze_pend.py` 는 `송신 = 수신 − RTT` 로 지연을 상·하향에 귀속시킨다. §29 회귀 방지. **테스트망에서만 실행**
- `analyze_leak.py` · `cmp_v130.py` (`test-results/2026-09-10-v131/`) — 누수 판정(계단 횟수·마지막 평탄 구간, `nproc` 모드값 필터)과 **직전 버전 런과의 경과시간 정렬 대조**. 램프가 관측 창보다 길 때 기울기 대신 쓰는 방법이다
...
- `mf_soak_18m.sh` + `analyze_soak.py` (`test-results/2026-08-07-aging-1h/`) — **지속 부하 소크.** `RATE=<앱 각 방향 Mbps>` 로 레이트 지정, 운영 체인(:443) 사용. 가드를 스스로 띄우고 precheck 실패 시 시작하지 않으며, r 의 `apt-daily-upgrade.timer` 를 정지했다가 복구한다(배경 유지보수가 1코어를 포화시켜 오탐 트립을 만든다). ⚠️ **가드가 트립하면 클라이언트 최종 집계가 SIGKILL 로 유실된다** — 수신측 `s:/tmp/sk_srv.log` 를 반드시 함께 회수할 것(1초 해상도로 살아남는다). **테스트망에서만 실행**
- `mf_aging_1h.sh` + `analyze.py` (`test-results/2026-08-07-aging-1h/`) — 다중 세션 저부하 에이징(병렬 체인 :4443, 운영 무침습). 누수 판정은 **마지막 30분 기울기**로 한다 — 램프가 약 20분이라 전반 기울기로 판정하면 정상을 누수로 오판한다. ⚠️ 정리 패턴의 포트 범위 `518[6-9]` 를 줄이지 말 것 — `518` 로 쓰면 **운영 클라이언트(:51821)까지 죽인다**(2026-08-07 실제로 20분 정지시켰다). **테스트망에서만 실행**
