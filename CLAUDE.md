...
```
  ...
  └────────┘      └──────────┘                   └────────────┘
   ...
   +enp3s0 192.168.1.214                          +ens20 1.220.235.146/29
...
[앱] → wg starlink-fec 10.9.10.2 (c) ←→ 10.9.10.1 (s)
       ...
       → multi-fec client -l 127.0.0.1:51821   (duplicate, mode 1, -f 5:1,20:4, --mtu 1350)
           ...
           └ path[1] → r 192.168.100.86:443  (multi-fec-relay@b)  ┘ gw 미통과
               → upstream → s 192.168.200.254:443                 ← gw 통과
...
192.168.200.0/24   via 192.168.100.1  dev enp2s0                     ★ 터널 아님
...
192.168.100.0/24   via 192.168.200.1  dev ens18                       ★ 터널 아님
...
|---|---|---|---|---|
| **c** | (self) | 온링크 | 온링크 | via .100.1 |
| **r** | 온링크 | (self) | (self) | via .100.1 |
| **s** | via .200.1 | via .200.1 | via .200.1 | (self) |
...
| 구간 | RTT | 손실 |
|---|---|---|
| WG 터널 `10.9.10.2 ↔ 10.9.10.1` | **54.3~54.9 ms** | 0% |
| s → r `.85`/`.86` | **1.00 ms** | 0% |
| c → s `.254` | **1.30 ms** | 0% |
...
**규칙 0 — 부하 시작 *전에* `mf_gwguard.sh watch` 를 띄운다.** gw `ens18` 방향당 **70 Mbps**
한도이고 대리지표는 **`s ens18` RX+TX**다. 손계산으로 "여유롭다" 판단하지 말 것 — 2026-08-03 에
그렇게 하고 가드 없이 돌려 **gw 가 800 Mbps 를 넘겼다**(원인 미확정, REPORT §3).
...
소스 IP 로 구분되지 않는다** — 경로별로 계측·임피어먼트를 걸 수 없다(현재 netem 은 온링크인
`c↔r` 구간에만 있다).
