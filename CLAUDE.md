...
MDS로 만드는 정합성 개선**으로서 유효하나(불필요한 복구실패 ~0.4% 제거), mode2 다운스트림
throughput 격차의 원인은 **여전히 미해결**(서버 로그상 다운스트림 fec/original≈2.1x 과다
오버헤드가 단서 — 추후 프로파일 필요). 측정 후 운영은 원본 바이너리+mode1로 원복.
