...
⚠️ **따라서 `mf_gwguard.sh` 와 70 Mbps 예산은 무효다.** 대리지표(`s ens18` RX+TX)가 gw 에
도달하지 않는 트래픽을 재고 있다. 과대보고(=일찍 트립)라 위험한 방향은 아니지만 보호 기능이
없는 것과 같다. **부하 상한은 이제 gw 가 아니라 호스트 CPU(c = Atom N2600)와 공유 스위치이며,
다음 부하 시험 전에 새 지표를 정해야 한다(아직 미정).** 아래 기록은 gw 경유 구성으로 되돌릴
때만 유효하다.
