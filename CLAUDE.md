...
**규칙 0 — ⚠️ 2026-08-05 이후 이 가드는 무효다.** `s` 리슨이 `192.168.100.84` 로 옮겨져
데이터 경로가 전부 온링크가 됐고 **gw 는 경로에 없다.** `mf_gwguard.sh` 는 `s ens18` RX+TX 를
gw 부하 대리로 쓰지만 그 트래픽은 이제 gw 에 도달하지 않는다 — 과대보고(=일찍 트립)라 안전한
방향이지만 **측정 대상이 틀렸다.** gw 를 경유하는 구성으로 되돌릴 때만 유효하다.

원래 취지(기록): gw `ens18` 방향당 **70 Mbps** 한도이고 대리지표는 **`s ens18` RX+TX**였다.
손계산으로 "여유롭다" 판단하지 말 것 — 2026-08-03 에 그렇게 하고 가드 없이 돌려 **gw 가
800 Mbps 를 넘겼다**(원인 미확정, REPORT §3). 새 구성의 병목은 gw 가 아니라 호스트 CPU 로
추정되며(c = Atom N2600), 부하 시험 전 대리지표를 다시 정해야 한다.
